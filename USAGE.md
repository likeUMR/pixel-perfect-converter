...
#### 实际 score 分布（基于 10 张测试图，均使用网格参考图生成）

| score 区间 | 张数 | 典型特征 |
|-----------|------|---------|
| **100** | 6 张 (60%) | AI 严格遵循了网格，格内近乎纯色（MCV < 200），BIR > 12，有可见网格线 |
| **70–80** | 1 张 (10%) | 有可见网格线，但部分格子颜色不够纯（MCV ≈ 4000），BIR ≈ 4 |
| **5–30** | 3 张 (30%) | 未检测到网格线，颜色连续渐变，BIR < 2 |

**关键发现：分布呈强双峰——要么接近满分（100），要么 5–25 分，70–95 之间几乎没有数据。**

**实际一次成功率约 60%**（得分 ≥ 90），即大约每 2 张图中有 1 张可以直接用。
...
倾向于追求高质量输出，而不是将就中间态——一张得 70 分的图意味着格子内颜色仍有较大噪点，转换后的像素块会有明显色差。得分 ≥ 90 的图格内方差极低（MCV < 200），转换结果几乎无损。

| score | 判断 | 建议操作 |
|-------|------|----------|
| ≥ 90 | 遵循良好 | 直接使用转换结果，质量有保障 |
| 60–89 | 部分遵循 | 可查看 report_card.png 确认，视需求决定；建议重新生成 |
| < 60 | 未遵循网格 | 重新生成 |
...
1. 生成一张图
2. 运行评估，查看 score
   ├── score ≥ 90（约 60% 概率）→ 直接使用 pixel_perfect_transparent.png / pixel_mini.png
   ├── score 60–89（约 10% 概率）→ 查看 report_card.png 确认，可选择接受或重新生成
   └── score < 60（约 30% 概率）→ 重新生成
```

**平均 2 次以内可得到一张高分图（≥ 90）。**

如果连续 3 次以上 score 都很低（< 30），通常说明 AI 没有理解网格意图，建议：
...
| `score` | 综合评分 = CPR_soft×40 + min(BIR/6,1)×60 | > 90 |

**综合评分解读（推荐接受线：90 分）：**

基于 10 张均使用网格参考图生成的测试图，score 分布呈强双峰：

- **100 分（60%）**：遵循网格，格内近乎纯色（MCV < 200），边界锐利（BIR > 12），**直接使用**
- **70–80 分（10%）**：有可见网格线，但格子内部仍有噪点（MCV ≈ 4000），转换后局部颜色有偏差，视精度需求决定
- **5–30 分（30%）**：未检测到网格线，颜色连续渐变，BIR < 2，转换结果不可用，应重新生成

> 30 到 90 分之间在测试数据中几乎没有图。推荐用 90 分作为接受线，追求接近无损的转换质量。
